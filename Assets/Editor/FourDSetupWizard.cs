...
public class FourDSetupWizard {
	...
	private static GameObject SetupMap4DObject() {
		...
		EnsureComponent<GenerateMap4DNearPlayer>(go, gen => {
			gen.ChunkSize = 4;
			gen.Range = 90f;  // BLOCK_SIZE=6, ChunkSize=4 → chunk=24m; 90m ≈ 3-4 chunks radius
		});
		...
		EnsureComponent<DistrictStyler>(go);
		EnsureComponent<DecorationPlacer4D>(go);
	...
	private static void WireReferences(GameObject map4DGO, GameObject playerGO) {
		...
		var groundPlane = map4DGO.GetComponent<GroundPlane>();
		if (groundPlane != null && groundPlane.playerTransform == null) {
			groundPlane.playerTransform = playerGO.transform;
		}

		// Always place player well above buildings (BLOCK_SIZE=6, MapHeight=6 → tops ~36m)
		playerGO.transform.position = new Vector3(0, 60f, 0);
