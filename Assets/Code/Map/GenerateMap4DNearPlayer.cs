...
public class GenerateMap4DNearPlayer : MonoBehaviour {
	...
	void Start() {
		...
		var controller = this.Target.GetComponent<IWPositionProvider>();
		if (controller != null) {
	...
	private void generate() {
		...
		int maxChunkW = Mathf.CeilToInt(GameState.NumWLayers / (float)this.ChunkSize) - 1;
		int wMin = Mathf.Max(0, chunkW - wRange);
		int wMax = Mathf.Min(maxChunkW, chunkW + wRange);
		...
		if (any) {
			if (this.VerboseLogging) Debug.Log($"[Gen4D] Queuing chunk {closestMissingChunk} (dist={closestDistance:F1}, wRange={wMin}..{wMax})");
	...
	private void createChunk(Vector4Int chunkAddress) {
		...
		var extent = new Vector4Int(this.ChunkSize, this.map.Height, this.ChunkSize, this.ChunkSize);
		if (this.VerboseLogging) Debug.Log($"[Gen4D] Collapsing chunk {chunkAddress} (slot start={start}, size={extent}, district={districtStyle})");
		this.map.Collapse(start, extent);
		lock (this.generatedChunks) {
			this.generatedChunks.Add(chunkAddress);
		}
		this.completedChunks4D.Enqueue(chunkAddress);
		if (this.VerboseLogging) Debug.Log($"[Gen4D] Chunk {chunkAddress} collapsed. Total chunks: {this.generatedChunks.Count}");
	}

	private void generatorThread() {
		if (this.VerboseLogging) Debug.Log("[Gen4D] Background generation thread started");
		int iterations = 0;
		try {
			while (true) {
				this.generate();
				iterations++;
				if (this.VerboseLogging && iterations % 20 == 0) Debug.Log($"[Gen4D] Thread iteration {iterations}, chunks total: {this.generatedChunks.Count}");
				Thread.Sleep(50);
			}
		} catch (Exception exception) {
			if (exception is ThreadAbortException) {
				if (this.VerboseLogging) Debug.Log("[Gen4D] Background thread stopped");
				return;
			}
