...
public class Slot4D {
	...
	// -1 = no bias; (int)ArchStyle otherwise — set before collapse by createChunk
	public int DistrictStyle = -1;
	...
	public void CollapseRandom() {
		...
		foreach (var module in this.Modules) {
			max += module.Prototype.Probability * this.DistrictWeight(module);
		}
		float roll = (float)(AbstractMap4D.Random.NextDouble() * max);
		float p = 0;
		foreach (var candidate in this.Modules) {
			p += candidate.Prototype.Probability * this.DistrictWeight(candidate);
			if (p >= roll) {
	...
	private float DistrictWeight(Module module) {
		if (this.DistrictStyle < 0 || module.DistrictWeights == null || this.DistrictStyle >= module.DistrictWeights.Length) {
			return 1f;
		}
		return module.DistrictWeights[this.DistrictStyle];
	}
